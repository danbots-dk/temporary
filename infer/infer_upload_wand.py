...
import sys
import os
from upload_point_cloud import upload_simulation_ply, upload_wand_ply
...
def main():
    # inFolder = '/danbots/data2/data/simulations/Planes/20230607/15h49m13s/'
    inFolder = '/home/samir/sal_github/docker/inference-dev-server/docker/sim_data/'
    myname = 'Kazi'
    ...
    wrp_folder = '/danbots/data2/data/models/nn1-1/20230824/15h30m56s/model'
    ...
    wrap_model_predict = tf.keras.models.load_model(wrp_folder + '.h5', compile=False)
    depth_model_predict = tf.keras.models.load_model(dp_folder + '.h5', compile=False)
    modelID = modelid
...
'''%(len(points),"".join(points))
    ...
    def wandinfer():
        ...
        for i in range(start,stop,step):
            ...
            inpfile = inFolder+'/render'  + str(i)+ '/sqimage.png'
            inpfile2 = inFolder+'/render'  + str(i)+ '/image8.png'
            col2 = cv2.imread(inpfile).astype(np.float32)
            col2 = cv2.resize(col2, (160,160), interpolation = cv2.INTER_AREA)
            cv2.imwrite(inFolder+'/render'  + str(i)+ '/image8.png', (col2))
        ...
        responses = upload_wand_ply(files=flist, batch_name=myname, model=modelID,  depth_model=dp_folder, wrap_model=wrp_folder)
